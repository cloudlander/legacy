...
LRESULT CSystemTray::OnTrayNotification(UINT wParam, LONG lParam) 
{
    ...
    {    
		...
		g_WndVec.clear();
		EnumWindows(EnumProc,(LPARAM)0);
		int size = g_WndVec.size();
		if(size > 0)
			pSubMenu->InsertMenu(0,MF_BYPOSITION|MF_SEPARATOR,0);
		// insert all the window menu items 
		int i;
		for(i=0;i<size;i++)
		{
			pSubMenu->InsertMenu(0,MF_BYPOSITION,ID_MENUITEM_BASE+i,g_WndVec[i].caption);
			if(g_WndVec[i].bTopmost)
				pSubMenu->CheckMenuItem(ID_MENUITEM_BASE+i,MF_BYCOMMAND|MF_CHECKED);		
		}
		CString str;
		str.Format("%d Topmost Windows",size);
		SetTooltipText(str);

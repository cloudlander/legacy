...
/* MD5 block update operation. Continues an MD5 message-digest
  ...
  context.
 */
void MD5::SessionUpdate (unsigned char *input, unsigned int inputLen)
{
  ...
  if ((ctx.count[0] += ((UINT4)inputLen << 3))
   < ((UINT4)inputLen << 3))
 ctx.count[1]++;
  ctx.count[1] += ((UINT4)inputLen >> 29);
...
*/
  if (inputLen >= partLen) {
 MD5_memcpy
   ((POINTER)&ctx.buffer[index], (POINTER)input, partLen);
 ...
 i = 0;
  ...
  MD5_memcpy
 ((POINTER)&ctx.buffer[index], (POINTER)&input[i],
  inputLen-i);
}
 ...
 */
void MD5::SessionFinal (unsigned char digest[16])
{
...
*/
  ...
  SessionUpdate (MD5::PADDING, padLen);

  /* Append length (before padding) */
  SessionUpdate (bits, 8);
  /* Store state in digest */
  Encode (digest, ctx.state, 16);

  /* Zeroize sensitive information.
*/
  MD5_memset ((POINTER)&ctx, 0, sizeof (ctx));
}

...
CMatrixT<T> CMatrixT<T>::operator+(CMatrixT<T>& right)
{
	...
	for(i=0;i<RowDim;i++)
	{
		for(j=0;j<ColDim;j++)
		{
			temp.SetAt(i,j,right.GetAt(i,j)+MatrixRow[i][j]);
		}
	}
...
template <class T>
CMatrixT<T> CMatrixT<T>::operator*(const T& right)
{
	int i,j;
	CMatrixT<T> temp(RowDim,ColDim);
	...
	for(i=0;i<RowDim;i++)
	{
		for(j=0;j<ColDim;j++)
		{
			temp.SetAt(i,j,MatrixRow[i][j]*right);
		}
	}
	return CMatrixT<T>(temp);
}
...
CMatrixT<T> CMatrixT<T>::operator*(CMatrixT<T>& right)
{
	int i,j,k;
	T s=0;
	CMatrixT<T> temp(RowDim,right.ColDim);
	for(i=0;i<RowDim;i++)
	{
		for(j=0;j<right.ColDim;j++)
		{
			for(s=0,k=0;k<ColDim;k++)
			{
				// just for double type
				if(MatrixRow[i][k]<0.0001 && MatrixRow[i][k]>-0.0001 || right.GetAt(k,j)<0.0001 && right.GetAt(k,j)>-0.0001)
					continue;

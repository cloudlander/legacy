...
//
// See project Readme.txt file for Terms of Use and Acknowledgements.
//
/////////////////////////////////////////////////////////////////////////////
/****************************************************************************
 *
 * $Date: 7/25/99 10:00p $
 * $Revision: 10 $
 * $Archive: /CodeJockey/CJLibrary/CJToolBar.cpp $
 *
 ...
 ***************************************************************************/
/////////////////////////////////////////////////////////////////////////////
//:Associate with class "CCJToolBar"
...
//:Custom (document with CJToolBar)
struct CJX_COLORMAP
{
...
CWnd* CCJToolBar::InsertControl( CRuntimeClass* pClass, LPCTSTR lpszWindowName, CRect& rect, UINT nID, DWORD dwStyle )
{
	...
	// CCJFlatComboBox control.
	if( pClass->IsDerivedFrom( RUNTIME_CLASS( CCJFlatComboBox ))) {
		pCtrl = new CCJFlatComboBox;
		ASSERT_VALID( pCtrl );
		if( !((CCJFlatComboBox*)pCtrl)->Create( WS_CHILD|WS_VISIBLE|dwStyle, rect, this, nID ))
		{
			delete pCtrl;
			return NULL;
		}
	...
	// An invalid object was passed in.
	else {
		ASSERT( FALSE );
		return NULL;
	}
...
void CCJToolBar::OnToolBarGetButtonInfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (0 <= tbStruct->iItem && tbStruct->iItem < m_nButtons)
	{
		...
		strcpy(tbStruct->pszText, m_pBarInfo[tbStruct->iItem].btnText);

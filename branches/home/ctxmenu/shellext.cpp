...
CShellExt::CShellExt()
{
   ...
   CFile of;
   CFileException ex;
   if(ERROR_SUCCESS==GetRegKey(HKEY_LOCAL_MACHINE, _T("Software\\EasyMD5"), buf))
   {
	   m_exePath=CString(_T("\""))+buf;
	   m_exePath+=_T("\\wmd5.exe\"");
	   return;
	   /*
	   AfxMessageBox(m_exePath);
		if(TRUE==of.Open(m_exePath,CFile::modeRead,&ex))
		{
			if(TRUE==of.GetStatus(stat))
			{
			}
		}
		*/
   ...
   errMsg+=_T("\nFatal error(");
   errMsg+=m_exePath;
   errMsg+=_T(", Please reinstall EasyMD5 software");

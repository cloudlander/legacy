 ...
 * translation to x86 as part of final code generation.
 *
...
class CodeGenerator {
  ...
  public:
         ...
         // In a x86 stack frame, first local is at ebp-4, subsequent locals
         // are at ebp-8, ebp-12, and so on. The first param is at ebp+8,
         // subsequent ones as ebp+12, ebp+16, etc. Saved %ebp and
         // retrun address are stored at ebp, ebp+4.
         // (Because methods have secret "this" passed in first param slot at
         // ebp+8, all normal params are shifted up by 4.)
         // Conveniently, all vars are 4 bytes in size for code generation
    static const int OffsetToFirstLocal = -4, OffsetToFirstParam = 8;
...
//   void GenAssign(Location *dst, Location *src);
    ...
    BeginMainFunc *GenBeginMainFunc();
	...
	void GenGlobalVar(const char* var);
         ...
         // flag tac is on (-d tac), it will not translate to x86,

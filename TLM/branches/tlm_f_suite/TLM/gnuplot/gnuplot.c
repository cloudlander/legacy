#ifndef lint
static char *RCSid() { return RCSid("$Id: pgnuplot.c,v 1.15.2.1 2007/08/26 06:15:51 sfeam Exp $"); }
#endif
...
cbGetTextWindow(HWND  hwnd, LPARAM  lParam)
{
    ...
    return (hwndText == NULL || piProcInfo.dwThreadId!=GetWindowThreadProcessId(hwndParent,NULL));
}
...
# define _setmode setmode /* this is for BC4.5 ... */
	...
	PostString(hwndText, psBuffer);
        Sleep(1);
        while(IsHungAppWindow(hwndParent))
          printf("Plotting\n");
          Sleep(1);
    }

...
!网格单位长度，起始网格位置结束网格位置，边界条件标志参数
      DOUBLE PRECISION, ALLOCATABLE:: RVB(:,:,:,:),IVB(:,:,:,:)
      DOUBLE PRECISION, ALLOCATABLE:: REEY(:,:,:,:),IMEY(:,:,:,:)
     
      DOUBLE PRECISION SB(5,5)
      ...
      DOUBLE PRECISION PANGLE !输入脉冲直线与Z轴张角(角度参数)
      ...
      !读入数据固定参数计算结构参数时使用      
      READ(11,*)SX,SY,SZ,ENDX,ENDY,ENDZ,X,Y,Z,U,V,W,H,ER,UR,SGM,NT,BL,SPLIT,SIDE1,SIDE2,ER1,ER2,NANGLE,GTYPE,COL,FRE,NORMAL_FRE,W0,PN,PSX,PSZ,PENDZ,PANGLE,LN,BP,SCALE,DO_NORMALIZE
      ...
      START_ROW=SX
      START_COL=NINT(SZ+(ENDZ-SZ)*(1-PART)/2)
	  IF(START_COL .LT. SZ) THEN  !检查介质范围是否有效
	    WRITE(*,*) "ERROR: MEDIUM OVERLAPPED THE SIMULATION AREA!"
		STOP
	  ENDIF
      GRID_ROW=NINT(REAL((ENDX-SX+1))/SIDE1)
      GRID_COL=NINT(REAL((ENDZ-SZ)*PART)/SIDE2)
...
!				   CALL COLOR(GTYPE,SIDE1,SIDE2,START_ROW+II*SIDE1,START_COL+KK*SIDE2,YY,YYY,NX,NZ,SX,ENDX,START_COL,END_COL,2*(U*W*SQRT(H)*ER2/V-2),ER2_COLOR, 1.0)
				   CALL COLOR(GTYPE,SIDE1*4,SIDE2*4,II,KK,YY,YYY,NX,NZ,SX,ENDX,START_COL,END_COL,2*(U*W*SQRT(H)*ER2/V-2),ER2_COLOR, 1.5)
!			ENDIF
!	 3332    CONTINUE
!	 3331 CONTINUE
                KK=KK+8*SIDE2
...
11111 CONTINUE
   ...
   1  CONTINUE
    ...
    !            IVB(III,PULSE(PIND,1),PULSE(PIND,2),PULSE(PIND,3))=IVB(III,PULSE(PIND,1),PULSE(PIND,2),PULSE(PIND,3))+sin(2*PI*(NORMAL_FRE/(SIDE1*4))*T)  !为sin激发单色波形式
